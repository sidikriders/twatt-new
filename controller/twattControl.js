...
function newTwat(req, res, next) {
  let mentah = req.body.tweet.split('')
  let newTweet = mentah.map(x=> {
    if (x === ' ') {
      return '%20'
    } else {
      return x
  ...
  oauth.post(
      `https://api.twitter.com/1.1/statuses/update.json?status=${newTweet.join('')}`,
      '320621031-a9AL3hth99QLzz5Ybq7HkQfqqm0Q94pxd8lHoxor', //test user token
      'q9rkQmtdrWk6xrJQBxbNwgEUgNyDkxGSbD69dDcr7Fgjx', //test user secret
      `${req.body.tweet}`,
      `text`,
